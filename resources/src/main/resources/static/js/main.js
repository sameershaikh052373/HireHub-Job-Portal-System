...
function filterJobs() {
    ...
    jobCards.forEach(card => {
        ...
        // Filter by category
        if (category !== '') {
            const jobTitle = card.querySelector('.card-title a').textContent.toLowerCase();
    ...
    if (sortBy === 'title') {
        ...
        visibleCards.sort((a, b) => {
            const dateA = new Date(a.querySelector('.text-muted').textContent.replace('Posted ', ''));
            const dateB = new Date(b.querySelector('.text-muted').textContent.replace('Posted ', ''));
            return dateB - dateA;
        });
